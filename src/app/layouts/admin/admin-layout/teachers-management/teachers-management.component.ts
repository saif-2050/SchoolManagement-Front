...
import { tDeletepopupComponent } from './tdeletepopup/tdeletepopup.component';

@Component({
  selector: 'frontend-teachers-management',
...
export class TeachersManagementComponent implements OnInit {
  ...
  FunctionDelete(id: any) {
    const dialogRef = this.dialog.open(tDeletepopupComponent,{
      data:{
    ...
    dialogRef.afterClosed().subscribe((confirmed: boolean) => {
      if (confirmed) {
        this.auth.RemoveTeacher(id).subscribe(result  => {
        this.rel = result
        this.getallteachers();
     });
      }
